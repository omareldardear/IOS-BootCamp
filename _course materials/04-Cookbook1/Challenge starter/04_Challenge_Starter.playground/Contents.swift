...
//:
//: __TODO 1 of 3:__ Extend the `Post` struct
...
extension Post {
  // TODO: Optional init: initialize Post properties or return nil
...
//: convert each dictionary into a Model object. Here, we have the optional `init`,
//: so we can create `Post` objects by passing each dictionary to it.
//:
//: __TODO 2 of 3:__ Create the task, with `url` and completion handler (rearrange and adapt the demo code):
class APIManager {
    ...
    func updateSearchResults(_ data: Data) {
        ...
        do {
            ...
            for postDictionary in response! as [JSONDictionary]{
                if let id = postDictionary["id"] as? Int,
                    ...
                    errorMessage += "Problem parsing PostDictionary\n"
                }
